/*
 * dcsd_spmv.hpp: DCSD SpMV implementation
 * (c) Mohammad Mofrad, 2018
 * (e) m.hasanzadeh.mofrad@gmail.com 
 */

#ifndef DCSD_SPMV_HPP
#define DCSD_SPMV_HPP

#include <chrono>

#include "pair.hpp" 
#include "io.cpp" 
#include "dcsd_base.hpp" 

class DCSD {
    public:
        DCSD() {};
        DCSD(const std::string file_path_, const uint32_t nvertices_, const uint32_t niters_) 
    ...
    protected:
        ...
        std::vector<char> diags;
        uint32_t nnzdiags_ = 0;
...
void DCSD::diag_sort(std::vector<struct Pair>*) {
  ...
  for(uint32_t i = 0; i < m; i++) {
    ...
    */
    ...
    //dcsd = new struct dcsd_BASE(nedges, nvertices);
    //dcsd = new struct dcsd_BASE(m^2, n);
/*    
    uint32_t* A  = (uint32_t*) dcsd->A;  // WEIGHT      
    uint32_t* IA = (uint32_t*) dcsd->IA; // ROW_IDX
    uint32_t* JA = (uint32_t*) dcsd->JA; // COL_PTR
    uint32_t ncols = dcsd->ncols - 1;
...
void DCSD::run_pagerank() {
    ...
    diag_sort(pairs);
    construct_filter();
    dcsd = new struct DCSD_BASE(nedges, nnzdiags_);
    //dcsd = new struct CSD_BASE(nedges, nvertices);
    populate();
...
void DCSD::construct_filter() {
    rows.resize(nvertices);
    diags.resize(ndiagonals);
    for(auto &pair: *pairs) {
        rows[pair.row] = 1;
        diags[ndiagonals - (pair.row - pair.col + nrows_global)] = 1;
    }
    for(uint32_t i = 0; i < ndiagonals; i++) {
        if(diags[i] == 1)
            nnzdiags_++;
    }
    //printf("nnzdiags_=%d\n", nnzdiags_);
...
void DCSD::populate() {
    ...
    uint32_t* IA = (uint32_t*) dcsd->IA; // ROW_IDX
    uint32_t* JA = (uint32_t*) dcsd->JA; // COL_PTR
    uint32_t* JC = (uint32_t*) dcsd->JC; // COL_IDX
    uint32_t* JI = (uint32_t*) dcsd->JI; // COL_IDX
    //uint32_t ncols = dcsd-> ncols - 1;
    uint32_t i = 0;
    uint32_t j = 1;
    JA[0] = 0;
    auto &p = pairs->front();
    JC[0] = ndiagonals - (p.row - p.col + nrows_global);
//k = ( j + IA[i] + 1) - nrows_global;
//(ndiagonals - (pair.row - pair.col + nrows_global)
    for(auto& pair : *pairs) {
        //printf("%d %d %d %d\n", pair.row, pair.col, ndiagonals - (pair.row - pair.col + nrows_global), j);
        if(JC[j - 1] != (ndiagonals - (pair.row - pair.col + nrows_global))) {
            JC[j] = (ndiagonals - (pair.row - pair.col + nrows_global));
            j++;
            JA[j] = JA[j - 1];
        }      
        A[i] = 1;
...
void DCSD::walk() {
    uint32_t* A  = (uint32_t*) dcsd->A;
    uint32_t* IA = (uint32_t*) dcsd->IA;
    uint32_t* JA = (uint32_t*) dcsd->JA;
    //uint32_t* JI = (uint32_t*) csd->JI;
    uint32_t ncols = dcsd->nnzcols;
    /*
    for(uint32_t j = 0; j < ncols; j++) {
        printf("j=%d\n", j);
        for(uint32_t i = JA[j]; i < JA[j + 1]; i++) {
...
void DCSD::message() {
    /*
    uint32_t* JC = (uint32_t*) dcsd->JC;
    uint32_t nnzcols = dcsd->nnzcols;
...
uint64_t DCSD::spmv() {
    ...
    */
    ...
    //ndiagonals
    for(j = 0; j < nnzdiags_; j++) {
        
        //printf("j=%d\n", j);
        for(uint32_t i = JA[j]; i < JA[j + 1]; i++) {
            k = (JC[j] + IA[i] + 1) - nrows_global;
            //l = j%(k+1);
...
void DCSD::display(uint64_t nums) {
    for(uint32_t i = 0; i < nums; i++)
        std::cout << "V[" << i << "]=" << v[i] << std::endl;
}

void DCSD::stats(double time, std::string type) {
